...
let tokenRequestPromise: Promise<GoogleDriveToken> | null = null;
...
export async function requestGoogleDriveToken(options: GoogleDriveTokenRequestOptions = {}): Promise<GoogleDriveToken> {
  ...
  tokenRequestPromise = (async () => {
    ...
    return new Promise<GoogleDriveToken>((resolve, reject) => {
      let settled = false;
      const timeoutId = window.setTimeout(() => {
        if (!settled) {
          settled = true;
          reject(new Error("Google sign-in did not finish. The popup may have been closed."));
        }
      }, 120000);
      ...
      const client = oauth2.initTokenClient({
        ...
        error_callback: (error) => {
          finish(() => reject(error instanceof Error ? error : new Error("Google sign-in failed.")));
        },
      });
      ...
      try {
        ...
        finish(() => reject(error instanceof Error ? error : new Error("Google sign-in could not start.")));
      }
    });
  })();
